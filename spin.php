...
$symbol_values = ['2.png' => 50, '10.png' => 30, '1.png' => 10, '3.png' => 8, '4.png' => 5, '5.png' => 4, '6.png' => 3, '7.png' => 2, '8.png' => 1];
...
$scatter_count = 0; $free_spins = 0;
if ($bet_amount > 0.01) { 
    foreach ($reels as $column) { foreach ($column as $symbol) { if ($symbol === '9.png') $scatter_count++; } }
    if ($scatter_count >= 3) $free_spins = 10;
}

$win_amount = 0; $is_win = false; $win_symbol = "";
foreach ($symbol_values as $sym => $multiplier) {
    $c1 = count(array_keys($reels[0], $sym)); $c2 = count(array_keys($reels[1], $sym)); $c3 = count(array_keys($reels[2], $sym));
    if ($c1 > 0 && $c2 > 0 && $c3 > 0) { 
        $is_win = true; $win_symbol = $sym;
        $c4 = count(array_keys($reels[3], $sym)); $c5 = count(array_keys($reels[4], $sym));
        $ways = $c1 * $c2 * $c3; $win_mult = 1.5; 
        if ($c4 > 0) { $ways *= $c4; $win_mult = 4; if ($c5 > 0) { $ways *= $c5; $win_mult = 10; } }
        $base_bet = ($bet_amount > 0) ? $bet_amount : 10;
        $win_amount += ($base_bet / 20) * $multiplier * $ways * $win_mult;
    }
}
