<?php
include 'db.php';
header('Content-Type: application/json');

$user_id = isset($_GET['uid']) ? intval($_GET['uid']) : 1;
$bet = 10.00;
$results = [];

$res = $conn->query("SELECT balance FROM users WHERE id = $user_id");
$user = $res->fetch_assoc();
$balance = $user['balance'];

for ($s = 0; $s < 50; $s++) {
    if ($balance < $bet) break;
    $balance -= $bet;
    
    // গ্রিড তৈরি (৫ রীল, ৪ রো)
    $reels = [];
    for ($i = 0; $i < 5; $i++) {
        $col = [];
        for ($j = 0; $j < 4; $j++) {
            $col[] = ['s' => rand(1, 10).".png", 'g' => (rand(1, 100) <= 15)];
        }
        $reels[] = $col;
    }
    // --- নিখুঁত উইন ক্যালকুলেশন (ভুল হাইলাইট বন্ধ হবে) ---
    $win_amount = 0;
    $win_pos = [];
    
    for ($r = 0; $r < 4; $r++) {
        $sym = $reels[$r]['s']; // [0] হলো ১ম কলাম, [$r] হলো নির্দিষ্ট রো

        $match = [[0, $r]];
        
        for ($c = 1; $c < 5; $c++) {
            $found_in_col = false;
            for ($row = 0; $row < 4; $row++) {
                // একই সিম্বল অথবা wild চেক
                if ($reels[$c][$row]['s'] === $sym || $reels[$c][$row]['s'] === 'wild.png') {
                    $match[] = [$c, $row];
                    $found_in_col = true;
                }
            }
            if (!$found_in_col) break; // যদি মাঝখানে কোনো কলামে না মিলে, তবে লুপ বন্ধ
        }

        // ৩টি বা তার বেশি মিললে তবেই উইন হবে
        if (count($match) >= 3) {
            $win_amount += count($match) * 5;
            foreach ($match as $m) {
                // পজিশনকে ইউনিক আইডি হিসেবে পাঠানো (c=column, r=row)
                $win_pos[$m[0].'-'.$m[1]] = ['c' => $m[0], 'r' => $m[1]];
            }
        }
    }

        

    $balance += $win_amount;
    $results[] = [
        'reels' => $reels,
        'win_pos' => array_values($win_pos),
        'win' => number_format($win_amount, 2, '.', ''),
        'bal' => number_format($balance, 2, '.', '')
    ];
}

$conn->query("UPDATE users SET balance = $balance WHERE id = $user_id");
echo json_encode(['results' => $results]);
