...
function generateRandomReels($card_paytable) {
    ...
    for ($col = 0; $col < 5; $col++) {
        ...
        for ($row = 0; $row < 4; $row++) {
            $r = rand(1, 100);
            if ($r <= 5) $img = "9.png"; elseif ($r <= 12) $img = "wild.png";
...
// অটো-রিফিল লজিক (১৫ এর নিচে নামলে ব্যাকগ্রাউন্ডে নতুন ৪০টি লোড হবে)
...
if ($total_left <= 15) {
    ...
    $big_win_idx = array_slice($indexes, 0, 8);
    $small_win_idx = array_slice($indexes, 8, 12);
    ...
    for ($i = 0; $i < $total_spins; $i++) {
        ...
        for ($c = 0; $c < 5; $c++) {
            for ($r = 0; $r < 4; $r++) {
                if (isset($reels[$c][$r]['s']) && $reels[$c][$r]['s'] === '9.png') { $scatter_pos[] = "$c,$r"; $sc_count++; }
            }
        }
